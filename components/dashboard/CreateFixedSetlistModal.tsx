  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40 p-4">
      <div className="bg-surface text-foreground rounded-lg shadow-lg w-full max-w-sm p-6 relative">
        ...
        <p className="text-sm text-muted mb-4">날짜에 상관없이 항상 목록에 고정되는 콘티입니다.</p>
        ...
        <form onSubmit={handleCreate} className="flex flex-col gap-4">
          ...
          <div className="flex gap-2 justify-end mt-2">
            ...
            <button
              type="submit"
              disabled={loading}
              className="bg-accent text-accent-foreground hover:bg-accent-hover rounded px-4 py-2 text-sm disabled:opacity-50"
            >
              {loading ? '생성 중...' : '콘티 생성'}
            </button>
          </div>
